...
function renderStarRating(rating10, styles) {
  if (typeof rating10 !== "number" || Number.isNaN(rating10)) {
    return null;
  }
...
function StarRatingInput({ value, onChange, ariaLabel, styles }) {
...
function LampToggle({ isDark, onToggle, styles }) {
  return (
    <button
      onClick={onToggle}
  ...
  return (
    <div className="App" style={{...styles.wrapper, color: theme.textPrimary}}>
      ...
      <div style={styles.lampToggleContainer}>
        <LampToggle isDark={isDarkMode} onToggle={toggleTheme} styles={styles} />
      </div>
                    ...
                    {(reviewsByBook[book.id] ?? []).length === 0 ? (
                      ...
                      <ul style={styles.reviewList}>
                        {reviewsByBook[book.id].map((review) => {
                          ...
                          const starNodes = renderStarRating(
